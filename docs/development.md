...
命令行输入、OpenAI-compatible streaming、tool call、打包和基础可用性。项目保持纯 CLI，
不提供 TUI 或全屏终端模式，也不提前实现还没有进入第一阶段的扩展能力。
...
PromptEvent 用于统一输入源和输入语义。stdin 和 mailbox 输入都可以映射为
...
当前 stdin/mailbox 沿用串行 idle 队列语义；真正的 `append_active` checkpoint 注入仍是
后续 M24 子任务。

MCP 侧 `mailbox_get` / `mailbox_wait` 继续只暴露最终 assistant output、状态和错误，
不返回 streaming delta 或 tool result 正文。

M24 不恢复 HTTP/WS product layer、daemon、registry、多 worker 或后台任务发布者进程。

Help/usage 是普通 CLI 行为。当前 help surface 支持：
