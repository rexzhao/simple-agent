...
> 历史说明：已完成里程碑保留其实现当时的行为。当前配置位置以
> `docs/configuration.md` 为准；Web 执行端使用统一 `server-root`，不再从项目
> `.agents/sai.yaml` 读取配置。
...
状态说明（2026-07-07）：M21 在实现前终止，HTTP/WS global daemon、registry 和 CLI client
方案不再是产品待办。原始未执行清单保存在
`docs/tasks/global-server-projects-checklist.md` 和 `docs/global-server-projects.md`，仅供历史参考；
它不应作为当前 backlog 解读。后续产品路径由 M22 收敛 execution library，并最终由 M25
替换为单可执行文件 Web 应用。
...
状态说明：M23 是已完成的历史 CLI 里程碑；M25 已删除 mailbox CLI 产品入口。
...
- [x] mailbox 新任务在 active stdin/mailbox turn 期间保持 queued，不打断当前 turn。
- [x] mailbox MCP result 继续只返回最终 assistant output、状态和错误。
- [x] 不恢复 HTTP/WS product layer、daemon、registry 或多 worker。
- [x] 支持 PromptEvent `append_active`，且只在 provider request、tool call 和 shell command
  之外的安全 checkpoint 生效。
- [x] 追加输入落盘为同一 `turn_id` 下的独立 user item。
- [x] 验证 `go test ./...`。
- [x] 验证 `git diff --check`。
...
当前执行目标。详细清单和验证边界见
`docs/tasks/v0.1-release-hardening-checklist.md`。

- [x] 归档 session 可在 Web UI 中查看、恢复或永久删除，不再形成无法返回的 UI 死路。
- [x] 浏览器 E2E 覆盖首次项目接入、session 创建、文本 streaming、持久化提交、cancel、
  run resync 和 session archive/restore。
- [x] M21-M24 历史方案与当前 backlog 明确分离，mailbox task-board proposal 标记为 superseded。
- [ ] 补齐 session/project 的日常管理 UX，包括 rename 和 project remove 的明确影响提示。
