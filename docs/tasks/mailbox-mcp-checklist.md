...
- [x] 目标命令形态改为 `sai --mailbox [127.0.0.1:PORT]`，并移除用户可见
  `--mailbox-mcp` 入口。
- [x] `sai --mailbox` 省略地址时自动监听 `127.0.0.1:0`，由 OS 分配可用端口。
- [x] 启动成功后向控制台打印实际 mailbox MCP URL。
- [x] 启动成功后写入 `.agents/${basename argv[0]}-mailbox.json` discovery 文件，包含
  endpoint URL、host、port、token、pid、命令 basename、started_at 和 protocol 信息。
- [x] mailbox MCP 请求使用 discovery 文件中的本地 capability token 进行鉴权。
...
- [x] CLI 开始/结束处理 mailbox task 时，在 stderr 打印包含 task id 的分隔符；结束
...
- [x] `sai --mailbox` 不带地址可启动 mailbox MCP，并在 stderr 打印实际监听 URL。
- [x] `sai --mailbox 127.0.0.1:PORT` 可启动指定本地地址。
- [x] `sai --mailbox` 启动后写入 `.agents/${basename argv[0]}-mailbox.json`，其他本地
  agent 可据此读取 endpoint 和 token。
- [x] 缺少或错误 token 的 mailbox MCP 请求被拒绝；正确 token 可正常 initialize/list/call。
- [x] CLI 正常退出时尽力删除 discovery 文件；启动时可覆盖同一 command basename 的 stale 文件。
...
- CLI default session 和 `session resume` 支持 `--mailbox [host:port]`；省略地址时自动选择
  `127.0.0.1` 和可用端口，并写入 mailbox discovery JSON；其他命令拒绝该 flag。
- mailbox discovery JSON 包含 endpoint URL、host、port、token、pid、命令 basename、
  started_at 和 protocol 信息；CLI 正常退出时尽力删除 discovery 文件。
- mailbox MCP 请求需要 discovery token；缺少或错误 token 的请求会被拒绝。
...
- `go test ./internal/cli` 覆盖 `--mailbox` root flag extraction、discovery/token、
  MCP tool result、wait timeout、queued cancel、running cancel。
- `go test ./...` 通过。
- `git diff --check` 通过。
