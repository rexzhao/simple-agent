// Pure TypeScript SHA-256. `crypto.subtle` is only exposed inside secure
// contexts, so pages served over plain HTTP on a LAN address (for example
// http://192.168.x.x) cannot use it. Blob integrity verification still needs
// a digest there, hence this dependency-free fallback.
...
export function sha256Hex(bytes: Uint8Array): string {
	...
	// The bit length always fits in 53 bits for in-memory buffers, so the high
	// 32 bits stay zero and the low 32 bits hold bitLength exactly.
