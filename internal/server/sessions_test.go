...
func TestSessionSendMessageIncrementalPersistsToolErrorAndContinues(t *testing.T) {
	root := filepath.Join(t.TempDir(), "sessions")
	store := sessions.NewV2Store(root)
	saveServerTestSession(t, store, "send-incremental-tool-error")
	runner := fakeIncrementalSessionTurnRunner{
		run: func(ctx context.Context, request SessionTurnRequest) (SessionTurnResult, error) {
			if request.TurnID == "" || request.Publisher == nil {
				t.Fatalf("runner turn id/publisher = %q/%T, want incremental request", request.TurnID, request.Publisher)
			...
			if err := request.Publisher.Publish(eventbus.AssistantReady{TurnID: request.TurnID, Message: model.Message{
				Role:    model.MessageRoleAssistant,
				Content: "final after tool error",
			}}); err != nil {
				return SessionTurnResult{}, err
			}
			return SessionTurnResult{Incremental: true}, nil
	...
	process := startSessionAPIServerWithTurnRunner(t, store, sessions.SessionV2{}, "registry-token", runner)
	conn := dialSessionStream(t, process, "send-incremental-tool-error")
	waitForStreamSubscribers(t, process, "send-incremental-tool-error", 1)

	raw, body := postRawJSONStatus(t, "http://"+process.Addr()+"/sessions/send-incremental-tool-error/messages", `{"content":"tool error prompt secret"}`, "registry-token", http.StatusOK)
	if body["status"] != "committed" || body["turn_id"] == "" || body["last_seq"] == nil {
		t.Fatalf("send response = %#v, want committed turn metadata", body)
	}
	for _, forbidden := range []string{"tool error prompt secret", "SECRET ARGUMENTS", "SECRET TOOL ERROR", "registry-token"} {
		if bytes.Contains(raw, []byte(forbidden)) {
			t.Fatalf("tool error response leaked %q: %s", forbidden, raw)
		}
	}
	...
	if events[5]["is_error"] != true || events[5]["content"] != nil || events[5]["arguments"] != nil {
		t.Fatalf("tool.finished event = %#v, want sanitized error result", events[5])
	}
	if events[7]["last_seq"] != body["last_seq"] {
		t.Fatalf("committed last_seq = %#v, response last_seq = %#v", events[7]["last_seq"], body["last_seq"])
	}
	for i, event := range events {
		...
		for _, forbidden := range []string{"tool error prompt secret", "SECRET ARGUMENTS", "SECRET TOOL ERROR"} {
			if bytes.Contains(raw, []byte(forbidden)) {
	...
	session, err := store.Load("send-incremental-tool-error")
	if err != nil {
		t.Fatalf("Load(send-incremental-tool-error) error = %v", err)
	}
	if session.RunningTurnID != "" || session.InterruptedTurnID != "" || !session.InterruptedAt.IsZero() {
		t.Fatalf("turn metadata = running %q interrupted %q at %s, want cleared successful turn", session.RunningTurnID, session.InterruptedTurnID, session.InterruptedAt)
	}
	if len(session.Items) != 4 {
		t.Fatalf("len(session.Items) = %d, want user+assistant+tool error+final: %#v", len(session.Items), session.Items)
	}
	userItem := session.Items[0]
	assistantItem := session.Items[1]
	toolItem := session.Items[2]
	finalItem := session.Items[3]
	if userItem.Message == nil || userItem.Message.Role != model.MessageRoleUser || userItem.Message.Content != "tool error prompt secret" {
	...
	if toolItem.Message == nil || toolItem.Message.Role != model.MessageRoleTool || toolItem.Message.ToolCallID != "call-error" || toolItem.Status != sessions.ItemStatusError || !toolItem.Message.IsError || toolItem.Message.Content != "SECRET TOOL ERROR" {
		t.Fatalf("tool item = %#v, want persisted error result", toolItem)
	}
	if finalItem.Message == nil || finalItem.Message.Role != model.MessageRoleAssistant || finalItem.Message.Content != "final after tool error" {
		t.Fatalf("final assistant item = %#v, want final response", finalItem)
	}
	if !reflect.DeepEqual(session.ActiveHistory, []string{userItem.ID, assistantItem.ID, toolItem.ID, finalItem.ID}) {
		t.Fatalf("ActiveHistory = %#v, want user+assistant+tool+final", session.ActiveHistory)
	}
	if events[1]["item_id"] != userItem.ID || events[2]["item_id"] != assistantItem.ID || events[3]["item_id"] != toolItem.ID || events[6]["item_id"] != finalItem.ID {
	...
	if len(messages) != 4 || messages[1].Role != model.MessageRoleAssistant || len(messages[1].ToolCalls) != 1 || messages[2].Role != model.MessageRoleTool || messages[2].ToolCallID != "call-error" || !messages[2].IsError || messages[2].Content != "SECRET TOOL ERROR" || messages[3].Content != "final after tool error" {
		t.Fatalf("materialized messages = %#v, want legal error tool exchange followed by final assistant", messages)
	}

	persisted, err := store.PersistedEventsAfter("send-incremental-tool-error", 0)
	if err != nil {
		t.Fatalf("PersistedEventsAfter() error = %v", err)
	}
...
func TestSessionSendMessageIncrementalFailureAfterCompactionKeepsCompactedHistory(t *testing.T) {
	root := filepath.Join(t.TempDir(), "sessions")
	store := sessions.NewV2Store(root)
	saveServerTestSession(t, store, "send-incremental-compact-fail")
	existingUser := appendServerTestItem(t, store, "send-incremental-compact-fail", sessions.SessionItem{
		ID:         "existing-user",
		Kind:       sessions.ItemKindMessage,
		Visibility: sessions.ItemVisibilityVisible,
		Audience:   sessions.ItemAudienceUser,
		Message:    &model.Message{Role: model.MessageRoleUser, Content: "existing prompt secret"},
	})
	existingAssistant := appendServerTestItem(t, store, "send-incremental-compact-fail", sessions.SessionItem{
		ID:         "existing-assistant",
		Kind:       sessions.ItemKindMessage,
		Visibility: sessions.ItemVisibilityVisible,
		Audience:   sessions.ItemAudienceModel,
		Message:    &model.Message{Role: model.MessageRoleAssistant, Content: "existing answer secret"},
	})
	if _, err := store.ReplaceActiveHistory("send-incremental-compact-fail", []string{existingUser.ID, existingAssistant.ID}); err != nil {
		t.Fatalf("ReplaceActiveHistory() error = %v", err)
	}

	summaryMessage := model.Message{Role: model.MessageRoleDeveloper, Content: "<compaction_summary>\npost compaction failure summary secret\n</compaction_summary>"}
	summary := sessions.SessionItem{
		ID:         "summary-1",
		Kind:       sessions.ItemKindMessage,
		Visibility: sessions.ItemVisibilityHidden,
	...
	runner := fakeIncrementalSessionTurnRunner{
		plan: func(ctx context.Context, request SessionTurnRequest) (SessionCompactionResult, error) {
			if request.TurnID == "" || request.Content != "after compact failure secret" {
				t.Fatalf("planner request turn/content = %q/%q, want turn and prompt", request.TurnID, request.Content)
			}
			if !reflect.DeepEqual(request.Session.ActiveHistory, []string{existingUser.ID, existingAssistant.ID}) {
				t.Fatalf("planner ActiveHistory = %#v, want existing active history", request.Session.ActiveHistory)
		...
		run: func(ctx context.Context, request SessionTurnRequest) (SessionTurnResult, error) {
			runnerCalled = true
			if !reflect.DeepEqual(request.Session.ActiveHistory, []string{summary.ID}) {
				t.Fatalf("runner ActiveHistory = %#v, want compacted history before user prompt snapshot", request.Session.ActiveHistory)
			}
			return SessionTurnResult{}, context.Canceled
		},
	}
	process := startSessionAPIServerWithTurnRunner(t, store, sessions.SessionV2{}, "registry-token", runner)
	conn := dialSessionStream(t, process, "send-incremental-compact-fail")
	waitForStreamSubscribers(t, process, "send-incremental-compact-fail", 1)

	raw, body := postRawJSONStatus(t, "http://"+process.Addr()+"/sessions/send-incremental-compact-fail/messages", `{"content":"after compact failure secret"}`, "registry-token", http.StatusInternalServerError)
	assertErrorCode(t, body, "turn_failed")
	if !runnerCalled {
		t.Fatal("runner was not called after compaction and turn input")
	}
	for _, forbidden := range []string{"after compact failure secret", "post compaction failure summary secret", "existing prompt secret", "existing answer secret", "registry-token"} {
		if bytes.Contains(raw, []byte(forbidden)) {
			t.Fatalf("post-compaction failure response leaked %q: %s", forbidden, raw)
		}
	}
	...
	for i, event := range events {
		...
		for _, forbidden := range []string{"after compact failure secret", "post compaction failure summary secret", "existing prompt secret", "existing answer secret"} {
			if bytes.Contains(raw, []byte(forbidden)) {
				t.Fatalf("event[%d] leaked %q: %s", i, forbidden, raw)
			}
		}
	...
	session, err := store.Load("send-incremental-compact-fail")
	if err != nil {
		t.Fatalf("Load(send-incremental-compact-fail) error = %v", err)
	}
	...
	if len(session.Items) != 4 {
		t.Fatalf("len(session.Items) = %d, want existing+summary+user without assistant/tool turn items: %#v", len(session.Items), session.Items)
	}
	if session.Items[0].ID != existingUser.ID || session.Items[1].ID != existingAssistant.ID || session.Items[2].ID != summary.ID {
		t.Fatalf("ledger prefix = %#v, want existing items plus summary", session.Items[:3])
	}
	userItem := session.Items[3]
	if userItem.Message == nil || userItem.Message.Role != model.MessageRoleUser || userItem.Message.Content != "after compact failure secret" {
		t.Fatalf("user item = %#v, want persisted post-compaction prompt", userItem)
	}
	if events[4]["item_id"] != userItem.ID {
		t.Fatalf("user item.appended event = %#v, want user item id %q", events[4], userItem.ID)
	}
	for _, item := range session.Items {
		if item.TurnID == turnID && item.Message != nil && item.Message.Role != model.MessageRoleUser {
			t.Fatalf("unexpected non-user turn item after post-compaction failure: %#v", item)
		}
		if item.Status == sessions.ItemStatusPending {
			t.Fatalf("unexpected pending item after post-compaction failure: %#v", item)
		}
	}
	if !reflect.DeepEqual(session.ActiveHistory, []string{summary.ID, userItem.ID}) {
		t.Fatalf("ActiveHistory = %#v, want compacted summary plus user prompt", session.ActiveHistory)
	}
	messages, err := session.MaterializeActiveHistory()
	if err != nil {
		t.Fatalf("MaterializeActiveHistory() error = %v", err)
	}
	if len(messages) != 2 || messages[0].Role != model.MessageRoleDeveloper || messages[0].Content != summaryMessage.Content || messages[1].Role != model.MessageRoleUser || messages[1].Content != "after compact failure secret" {
		t.Fatalf("materialized messages = %#v, want compacted summary plus user prompt", messages)
	}
	persisted, err := store.PersistedEventsAfter("send-incremental-compact-fail", 0)
	if err != nil {
		t.Fatalf("PersistedEventsAfter() error = %v", err)
	}
	for _, event := range persisted {
...
func TestSessionSendMessageIncrementalInterruptsWhenRunnerSkipsIncrementalPersistence(t *testing.T) {
	root := filepath.Join(t.TempDir(), "sessions")
	store := sessions.NewV2Store(root)
	saveServerTestSession(t, store, "send-incremental-runner-skips")
	runner := fakeIncrementalSessionTurnRunner{
		run: func(ctx context.Context, request SessionTurnRequest) (SessionTurnResult, error) {
			if request.TurnID == "" || request.Publisher == nil {
				t.Fatalf("runner turn id/publisher = %q/%T, want incremental request", request.TurnID, request.Publisher)
	...
	process := startSessionAPIServerWithTurnRunner(t, store, sessions.SessionV2{}, "registry-token", runner)
	conn := dialSessionStream(t, process, "send-incremental-runner-skips")
	waitForStreamSubscribers(t, process, "send-incremental-runner-skips", 1)

	raw, body := postRawJSONStatus(t, "http://"+process.Addr()+"/sessions/send-incremental-runner-skips/messages", `{"content":"runner skipped incremental secret"}`, "registry-token", http.StatusInternalServerError)
	assertErrorCode(t, body, "turn_runner_error")
	if message := body["error"].(map[string]any)["message"]; message != "turn runner did not use incremental persistence" {
		t.Fatalf("error message = %#v, want incremental persistence error", message)
	}
	for _, forbidden := range []string{"runner skipped incremental secret", "registry-token"} {
		if bytes.Contains(raw, []byte(forbidden)) {
			t.Fatalf("runner skip response leaked %q: %s", forbidden, raw)
		}
	}
	...
	for i, event := range events {
		...
		if bytes.Contains(raw, []byte("runner skipped incremental secret")) {
			t.Fatalf("event[%d] leaked prompt: %s", i, raw)
		}
	}

	session, err := store.Load("send-incremental-runner-skips")
	if err != nil {
		t.Fatalf("Load(send-incremental-runner-skips) error = %v", err)
	}
	if session.RunningTurnID != "" || session.InterruptedTurnID != "turn-000001" || session.InterruptedAt.IsZero() {
		t.Fatalf("turn metadata = running %q interrupted %q at %s, want interrupted turn-000001", session.RunningTurnID, session.InterruptedTurnID, session.InterruptedAt)
	}
	...
	userItem := session.Items[0]
	if userItem.Message == nil || userItem.Message.Role != model.MessageRoleUser || userItem.Message.Content != "runner skipped incremental secret" {
		t.Fatalf("user item = %#v, want persisted prompt", userItem)
	}
	if events[1]["item_id"] != userItem.ID {
		t.Fatalf("item.appended event = %#v, want user item id %q", events[1], userItem.ID)
	...
	for _, item := range session.Items {
		if item.Message != nil && item.Message.Role == model.MessageRoleAssistant {
			t.Fatalf("unexpected assistant item after runner skipped incremental persistence: %#v", item)
		}
		if item.Message != nil && item.Message.Role == model.MessageRoleTool {
			t.Fatalf("unexpected tool item after runner skipped incremental persistence: %#v", item)
		}
		if item.Status == sessions.ItemStatusPending {
			t.Fatalf("unexpected pending item after runner skipped incremental persistence: %#v", item)
		}
	}
	messages, err := session.MaterializeActiveHistory()
	if err != nil {
	...
	if len(messages) != 1 || messages[0].Role != model.MessageRoleUser || messages[0].Content != "runner skipped incremental secret" {
		t.Fatalf("materialized messages = %#v, want persisted user prompt", messages)
	}
	persisted, err := store.PersistedEventsAfter("send-incremental-runner-skips", 0)
	if err != nil {
		t.Fatalf("PersistedEventsAfter() error = %v", err)
	}
	for _, event := range persisted {
